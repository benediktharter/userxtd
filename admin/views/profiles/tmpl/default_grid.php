...
foreach( $this->items as $k => $item ):
	...
	// =================================================================================
		...
		if ($item->get('a_checked_out'))
			$content .= JHtml::_('jgrid.checkedout', $i, $item->get('a_checked_out'), $item->get('a_checked_out_time'), 'profiles.', $canCheckin);
		
		if ($canEdit || $canEditOwn) 
			$content .= JHtml::link(JRoute::_('index.php?option=com_userxtd&task=profile.edit&id='.$item->a_id), $item->get('a_title')) ;
		
		
		$content .= '<div class="small">'.JText::sprintf('JGLOBAL_LIST_ALIAS', $this->escape( $item->get('a_alias') )).'</div></div>' ;
		
		JHtml::_('dropdown.edit', $item->id, 'profiles.');
		...
		if ($item->a_checked_out) :
			JHtml::_('dropdown.checkin', 'cb' . $i, 'profiles.');
		endif;
		
		
		if ($trashed) :
			JHtml::_('dropdown.untrash', 'cb' . $i, 'profiles.');
		else :
			JHtml::_('dropdown.trash', 'cb' . $i, 'profiles.');
		endif;
	...
	// =================================================================================
		...
		$content = JHtml::_('jgrid.published', $item->a_published, $i, 'profiles.', $canChange, 'cb', $item->a_publish_up, $item->a_publish_down);
