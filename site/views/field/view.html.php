 ...
 */
class UserxtdViewField extends AKViewItem
{
	/**
	...
	public function display($tpl = null)
	{
		...
		// =====================================================================================
		$item = $this->item ;
		$item->link = JRoute::_("index.php?option=com_userxtd&view=field&id={$item->id}&alias={$item->alias}&catid={$item->catid}");
		...
		if (!$user->get('guest')) {
			$userId	= $user->get('id');
			$asset	= 'com_userxtd.field.'.$item->id;
		...
		$results = $dispatcher->trigger('onContentPrepare', array ('com_userxtd.field', &$item, &$this->params, 0));

		$item->event = new stdClass();
		$results = $dispatcher->trigger('onContentAfterTitle', array('com_userxtd.field', &$item, &$this->params, 0));
		$item->event->afterDisplayTitle = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentBeforeDisplay', array('com_userxtd.field', &$item, &$this->params, 0));
		$item->event->beforeDisplayContent = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentAfterDisplay', array('com_userxtd.field', &$item, &$this->params, 0));
		...
		// Params
		// =====================================================================================
		// Merge field params. If this is single-field view, menu params override article params
		// Otherwise, field params override menu item params
		...
		if ($active) {
			...
			// If the current view is the active item and an field view for this field,
			// then the menu item params take priority
			if (strpos($currentLink, 'view=field') && (strpos($currentLink, '&id='.(string) $item->id))) {
				// $item->params are the field params, $temp are the menu item params
				...
				// Current view is not a single field, so the field params take priority here
				// Merge the menu item params with the field params so that the field params take priority
				$temp->merge($item->params);
				$this->params = $temp;

				// Check for alternative layouts (since we are not in a single-field menu item)
				// Single-field menu item layout takes priority over alt layout for an field
				if ($layout = $this->params->get('field_layout')) {
